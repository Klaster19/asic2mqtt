...
## Установка и запуск как системный демон (systemd)

Для запуска asic2mqtt как системного демона в Linux с использованием systemd:
...
2. Создайте пользователя и группу для демона (опционально, но рекомендуется):
   ```bash
   sudo useradd -r -s /bin/false asic2mqtt
   ```

3. Создайте директорию для конфигурационных файлов:
   ```bash
   sudo mkdir -p /etc/asic2mqtt
   ```

4. Скопируйте конфигурационный файл:
   ```bash
   sudo cp config_example.json /etc/asic2mqtt/config.json
   ```
   
5. Отредактируйте конфигурационный файл:
   ```bash
   sudo nano /etc/asic2mqtt/config.json
   ```

6. Скопируйте systemd unit файл:
   ```bash
   sudo cp asic2mqtt.service /etc/systemd/system/
   ```

7. Перезагрузите конфигурацию systemd:
   ```bash
   sudo systemctl daemon-reload
   ```

8. Включите автозапуск демона:
   ```bash
   sudo systemctl enable asic2mqtt
   ```

9. Запустите демон:
   ```bash
   sudo systemctl start asic2mqtt
   ```

10. Проверьте статус демона:
    ```bash
    sudo systemctl status asic2mqtt
    ```

11. Просмотр логов демона:
    ```bash
    sudo journalctl -u asic2mqtt -f
    ```

## Переменная окружения

Скрипт поддерживает переменную окружения `CONFIG_PATH`, которая позволяет указать путь к конфигурационному файлу. Это особенно полезно при запуске как системного демона.

Пример использования:
```bash
CONFIG_PATH=/etc/asic2mqtt/config.json asic2mqtt
```

При запуске через systemd переменная окружения уже настроена в unit файле.
...
- `asic2mqtt.service` - systemd unit файл для запуска asic2mqtt как системного демона
