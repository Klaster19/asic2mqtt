...
"""
Тестовый скрипт для проверки функциональности wm.py без подключения к реальным устройствам
"""
...
def test_wm_functionality():
    """Тест функциональности wm.py"""
    print("Тестирование функциональности wm.py")
    print("=" * 40)
    ...
    with patch('subprocess.run') as mock_subprocess, \
        ...
        try:
            # Импортируем и тестируем отдельные функции из wm.py
            import wm
            
            # Проверяем, что конфигурация загружена корректно
            print("✅ Импорт wm.py выполнен успешно")
            
            # Проверяем функцию проверки доступности хоста
            result = wm.is_host_available("192.168.3.34")
            print(f"✅ Функция is_host_available работает: {result}")
            
            # Проверяем функцию получения данных от Whatsminer
            summary, edevs = wm.get_whatsminer_data("192.168.3.34", mock_token_instance)
            ...
            stats, devs = wm.get_antminer_data("192.168.3.73", "root", "root")
...
if __name__ == "__main__":
    ...
    if success:
        print("\n✅ Тест функциональности wm.py пройден успешно!")
        sys.exit(0)
    else:
        print("\n❌ Тест функциональности wm.py не пройден!")
        sys.exit(1)